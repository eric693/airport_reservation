...
def _show_quote_result(reply_token, session, user_id):
    ...
    o.pet              = False
    o.booking_date     = session.get('quote_date', '')  # ← 先設定日期
    o.extra_stop_fee   = 0
...
Q6 小孩需要安全座椅嗎？可以提供嗎？
A：依法規定，四歲以下兒童需使用安全座椅。安全座椅／增高墊加收 NT$200／座，請提供幾歲用的。
...
Q28 請問小孩幾歲前需要坐安全座椅？
A：依法規定，四歲以下兒童需使用安全座椅。如需租借請在預約時選擇兒童安全座椅加購，加收 NT$200／座，謝謝您！
...
def build_quote_from_session(session):
    ...
    o.booking_date     = session.get('date', '')  # ← 先設定
